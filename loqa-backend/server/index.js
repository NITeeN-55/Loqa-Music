...
/* ── CORS ─────────────────────────────────────────────────────
   ...
   OPTIONS preflight requests get the correct headers even when
   they are rejected or rate-limited.
   ──────────────────────────────────────────────────────────── */
...
if (!IS_DEV && ALLOWED_ORIGINS.length === 0) {
  ...
  console.error('   Set it in Render → Environment: https://loqa-music.vercel.app');
}

const corsOptions = {
  origin(origin, callback) {
    // No origin = server-to-server request, curl, Render health checks → always allow
    if (!origin) return callback(null, true);
    // Dev → allow everything
    if (IS_DEV) return callback(null, true);
    // Prod → exact match against allowlist
...
/* ── Security ─────────────────────────────────────────────────
   Comes AFTER cors() so helmet never strips Access-Control-*
   headers that cors() just set.
   ──────────────────────────────────────────────────────────── */
app.use(helmet({
...
app.use((err, _req, res, _next) => {
  // Surface CORS errors clearly in logs
...
initDatabase().then(() => {
  const server = app.listen(PORT, '0.0.0.0', () => {
    ...
    if (!IS_DEV) {
      ...
      ALLOWED_ORIGINS.forEach(o => console.log(`║     ${o.padEnd(36)}║`));
    }
    console.log('╚══════════════════════════════════════════╝\n');
  });
